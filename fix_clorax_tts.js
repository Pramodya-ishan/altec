...
const target = /\{\/\* TTS Voice Feedback Control Toggle \*\/\}([\s\S]*?)<\/button>\s*\{isTtsAvailable && \(\s*<button([\s\S]*?)<\/button>\s*\)\}\s*<\/div>/;

p = p.replace(target, `{/* TTS Voice Feedback Control Toggle */}
                {isTtsAvailable && (
                  <button
                    type="button"
                    onClick={() => {
