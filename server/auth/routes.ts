...
function toPublicProfile(profile: any) {
  ...
  const { password, verificationCode, ...safeProfile } = profile;
  return safeProfile;
}
...
authRoutes.post("/email-login", async (req, res) => {
    try {
      ...
      res.json({ success: true, user: userSession, profile: toPublicProfile(savedProfile) });
    } catch (e: any) {
      res
        .status(500)
...
authRoutes.post("/register-start", async (req, res) => {
    try {
      ...
      res.json({
        ...
        user: userSession,
        profile: toPublicProfile(newProfile),
      });
    } catch (e: any) {
...
authRoutes.post("/verify-code", async (req, res) => {
    try {
      ...
      res.json({ success: true, user: userSession, profile: toPublicProfile(userData.profile) });
    } catch (e: any) {
