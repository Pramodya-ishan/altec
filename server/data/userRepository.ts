...
const emptyUser = () => ({ data: null, history: [], cookies: null, profile: null, notifications: [] });

function readUserFile(file: string) {
  const raw = fs.readFileSync(file);
  const unzipped = zlib.gunzipSync(raw).toString('utf-8');
  let jsonStr = unzipped;
  if (!jsonStr.trimStart().startsWith('{')) jsonStr = decrypt(jsonStr);
  const parsed = JSON.parse(jsonStr);
  return parsed && typeof parsed === 'object' ? parsed : emptyUser();
}

export function readUser(email: string) {
  const file = getUserFile(email);
  const backupFile = `${file}.bak`;
  if (!fs.existsSync(file) && !fs.existsSync(backupFile)) return emptyUser();

  try {
    if (fs.existsSync(file)) return readUserFile(file);
  } catch (error) {
    console.error(`Primary user database file is unreadable for ${email}:`, error);
  }

  try {
    if (fs.existsSync(backupFile)) {
      const recovered = readUserFile(backupFile);
      try { fs.copyFileSync(backupFile, file); } catch {}
      console.warn(`Recovered user database from atomic backup for ${email}.`);
      return recovered;
    }
  } catch (error) {
    console.error(`Backup user database file is unreadable for ${email}:`, error);
  }
...
export function writeUser(email: string, userData: any) {
  ...
  // Background serverless backup. The payload stays encrypted before leaving the server.
  if (email && email.includes('@') && apiKey) {
    ...
    fetch(url, {
      ...
      console.error('Failed to backup user data to Firestore REST API:', error);
    });
  }
}
