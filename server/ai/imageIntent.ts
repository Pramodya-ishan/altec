const IMAGE_NOUNS = /(?:image|picture|illustration|diagram|infographic|visual|chart|graph|රූප(?:ය|යක්|යක්ද)?|පින්තූර(?:ය|යක්)?|රූපසටහන|රූප සටහන|ප්‍රස්තාර(?:ය|යක්)?|සටහන(?:ක්)?)/iu;
const IMAGE_ACTIONS = /(?:create|generate|draw|make|design|show|visuali[sz]e|render|හදන්න|සාදන්න|අඳින්න|නිර්මාණය|පෙන්වන්න|රූපගත|hadanna|hadan|adinna|pennanna)/iu;
