...
import { readUser, writeUser, syncUserFromFirestore } from "./server/data/userRepository";
...
async function startServer() {
  ...
  // Restore a user's encrypted Firestore backup before account/data requests.
  // Failures are logged but never crash the whole serverless function.
  app.use(async (req, _res, next) => {
    try {
      const candidate = req.query.email || req.body?.email;
      const email = typeof candidate === 'string' ? candidate.trim().toLowerCase() : '';
      if (email.includes('@')) {
        await syncUserFromFirestore(email);
      }
    } catch (error) {
      console.error('User restore middleware failed:', error);
    }
    next();
  });

  // Keep authentication startup lightweight. Heavy AI/Firebase Admin modules are
  // loaded only when an AI endpoint is requested, preventing Vercel cold-start
  // crashes from taking down /api/auth/* as well.
  app.use('/api/auth', authRoutes);

  app.get('/api/health', (_req, res) => {
    res.json({ ok: true, runtime: process.version, vercel: Boolean(process.env.VERCEL) });
  });

  let aiRouterPromise: Promise<any> | null = null;
  app.use('/api', async (req, res, next) => {
    const isAiRoute = req.path.startsWith('/ai-chat') || req.path.startsWith('/chat');
    if (!isAiRoute) return next();
    try {
      aiRouterPromise ||= import('./server/ai/routes').then((m) => m.aiRoutes);
      const aiRouter = await aiRouterPromise;
      return aiRouter(req, res, next);
    } catch (error: any) {
      console.error('Failed to load AI routes:', error);
      return res.status(500).json({ error: 'AI service failed to start', details: error?.message || String(error) });
    }
  });
  ...
  app.get("/api/quota", async (_req, res) => {
    try {
      const queue = await import('./server/ai/queue');
      res.json({
        rpmUsed: queue.requestCountPM,
        rpmLimit: queue.RPM_LIMIT,
        rpdUsed: queue.requestCountPD,
        rpdLimit: queue.RPD_LIMIT,
      });
    } catch (e: any) {
  ...
  app.get("/api/yt-download", async (req, res) => {
    try {
      ...
      const play: any = playModule.default || playModule;
  ...
  app.use('/api', (err: any, _req: any, res: any, _next: any) => {
    ...
    res.status(500).json({ error: 'Internal API error', details: err?.message || String(err) });
  });
