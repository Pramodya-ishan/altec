...
// Progress writes use last-write-wins metadata and explicitly protect non-empty data from accidental empty overwrites.
const hasMeaningfulProgress = (data: any) => {
  if (!data || typeof data !== 'object') return false;
  return ['sft', 'et', 'ict'].some((key) => {
    const subject = data[key] || {};
    return Object.keys(subject.topics || {}).length > 0 ||
      (Array.isArray(subject.paperMarks) && subject.paperMarks.length > 0) ||
      Object.values(subject.questionMarks || {}).some((rows: any) => Array.isArray(rows) && rows.length > 0) ||
      (Array.isArray(subject.lessonHistory) && subject.lessonHistory.length > 0);
  }) || (Array.isArray(data.zScoreHistory) && data.zScoreHistory.length > 0) || data.targetZ !== undefined || Boolean(data.studyPlan);
};
...
async function startServer() {
  ...
  // Restore the encrypted serverless snapshot before routes read a user that is missing from ephemeral storage.
  app.use(async (req, _res, next) => {
    try {
      const rawEmail = (req.query.email || req.body?.email || '') as string;
      if (rawEmail && rawEmail.includes('@')) await syncUserFromFirestore(rawEmail.trim().toLowerCase());
    } catch (error) {
      console.warn('User snapshot restore skipped:', error);
    }
    next();
  });
  ...
  app.get('/api/data', (req, res) => {
    try {
      ...
      return res.json({
        ...
        updatedAt: userData?.dataUpdatedAt ?? null,
        revision: Number(userData?.dataRevision) || 0,
        reset: userData?.dataReset === true,
      });
    } catch (error) {
      console.error('Progress DB read failed:', error);
  ...
  app.post('/api/data', (req, res) => {
    try {
      const { email: rawEmail, data, updatedAt, revision, reset } = req.body || {};
      if (!rawEmail || !data || typeof data !== 'object') {
        return res.status(400).json({ error: 'Missing or invalid payload' });
      }

      const email = String(rawEmail).trim().toLowerCase();
      const userData = readUser(email);
      const incomingUpdatedAt = typeof updatedAt === 'string' ? updatedAt : new Date().toISOString();
      const incomingRevision = Number(revision) || 0;
      const existingTime = timestampValue(userData?.dataUpdatedAt);
      const incomingTime = timestampValue(incomingUpdatedAt);

      if (existingTime > incomingTime && userData?.data) {
        return res.status(409).json({
          error: 'Stale progress write rejected',
          updatedAt: userData.dataUpdatedAt,
          revision: Number(userData.dataRevision) || 0,
        });
      }

      if (!reset && !hasMeaningfulProgress(data) && hasMeaningfulProgress(userData?.data)) {
        return res.status(409).json({ error: 'Destructive empty overwrite rejected; explicit reset is required.' });
      }

      userData.data = data;
      userData.dataUpdatedAt = incomingUpdatedAt;
      userData.dataRevision = incomingRevision;
      userData.dataReset = reset === true;
      writeUser(email, userData);

      return res.json({ success: true, updatedAt: incomingUpdatedAt, revision: incomingRevision });
    } catch (error) {
      console.error('Progress DB write failed:', error);
      return res.status(500).json({ error: 'DB Error' });
    }
  });
