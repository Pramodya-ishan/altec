...
export default function PastPapersView() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!user || user.isAnonymous) {
      setToast({ type: 'error', message: "Please sign in before uploading a PDF." });
      return;
    }
    ...
    try {
      ...
      setToast({ type: 'success', message: "PDF uploaded successfully. Search indexing is in progress." });
    } catch (error: any) {
      console.error("Upload failed", error);
      setToast({ type: 'error', message: `Could not upload the PDF: ${error.message || error}` });
      setIsUploading(false);
      setUploadProgress(0);
      setUploadTelemetry(null);
      uploadControlsRef.current = null;
  ...
  const handleDeletePaper = async (paper: any, e: React.MouseEvent) => {
    ...
    try {
      ...
      setToast({ type: 'error', message: `Could not delete the paper: ${err.message}` });
    }
  };
 ...
 <p className="text-slate-400 text-sm">Upload a PDF or change your search.</p>
 </div>
 ) : (
 <motion.div 
 ...
 onClick={() => {
    openSourcePdf({ storagePath: paper.storagePath, url: paper.url, id: paper.id }).catch((e: any) => {
      ...
      if (e.message?.includes('LOGIN_REQUIRED')) {
         alert('Please sign in again to open this PDF.');
      } else if (e.message?.includes('storage/unauthorized')) {
         alert('PDF access was denied. Check your session and try again.');
      } else if (e.message?.includes('NOT_A_PDF_RESPONSE')) {
         alert('The server returned an invalid PDF response. Please try again.');
      } else if (e.message?.includes('NO_OPENABLE_PDF_SOURCE')) {
         alert('This source does not have an available PDF file.');
      } else {
