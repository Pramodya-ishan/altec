...
export function CloraXView() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (result.chatHistory && result.chatHistory.length > 0) {
            setMessages(result.chatHistory.map((m: any) => ({
              role: m.role,
              content: m.text || m.content,
              id: m.id || Math.random().toString(),
              summary: m.safeSummary || m.summary
            })));
          }
        } else {
          setHistoryWarning("Warning: Unable to load previous chat history. Your chats may not be saved.");
        }
      } catch (err: any) {
        console.warn("Failed to load chat history:", err);
        setHistoryWarning("Warning: Chat history saving is offline or unavailable.");
      }
    }
    loadHistory();
  }, []);
  ...
  useEffect(() => {
    if (!isStreaming && status?.stage === 'done' && answer && currentRequestIdRef.current) {
        const assistantMsgId = currentRequestIdRef.current;
        setMessages(prev => {
        ...
        currentRequestIdRef.current = null; // Reset ref so it won't append again

    }
  }, [isStreaming, status?.stage, answer, safeSummary]);
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-3xl mx-auto px-4 sm:px-6 py-4 bg-white md:bg-transparent">
      ...
      {historyWarning && (
        <div className="shrink-0 mb-4 p-3 bg-amber-50 border border-amber-200 rounded-2xl text-amber-800 text-xs font-semibold flex items-center justify-between shadow-sm animate-fade-in">
          <span className="flex items-center gap-2">
            <i className="fa-solid fa-triangle-exclamation text-amber-500 text-sm"></i>
            {historyWarning}
          </span>
          <button onClick={() => setHistoryWarning(null)} className="text-amber-400 hover:text-amber-600 cursor-pointer p-1">
