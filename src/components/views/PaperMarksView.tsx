...
export function PaperMarksView() {
  ...
  const maxScale = currentSubject === 'et' ? 70 : 100;
  const maxSubScale = currentSubject === 'et' ? 35 : 50;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex justify-end gap-3 items-center w-full relative z-10 mb-2">
        {/* Switch Toggle */}
        <div className="flex bg-slate-100 p-1 rounded-full gap-1 shadow-inner border border-slate-200 w-full sm:w-auto relative items-center">
          <button
            onClick={() => setMarksTab('tracker')}
            className={cn(
              "relative px-5 py-1.5 rounded-full text-xs font-display font-extrabold uppercase transition-colors duration-200 cursor-pointer flex-1 sm:flex-none text-center z-10",
              marksTab === 'tracker' ? "text-primary-700 font-bold" : "text-slate-500 hover:text-slate-900 hover:bg-slate-200/50"
            )}
          >
            {marksTab === 'tracker' && (
              <motion.div
          ...
          <button
            onClick={() => setMarksTab('mcq')}
            className={cn(
              "relative px-5 py-1.5 rounded-full text-xs font-display font-extrabold uppercase transition-colors duration-200 cursor-pointer flex-1 sm:flex-none text-center z-10",
              marksTab === 'mcq' ? "text-primary-700 font-bold" : "text-slate-500 hover:text-slate-900 hover:bg-slate-200/50"
            )}
          >
            {marksTab === 'mcq' && (
              <motion.div
      ...
      <AnimatePresence mode="wait">
        {marksTab === 'tracker' ? (
          ...
          <motion.div
            ...
            exit={{ opacity: 0, y: -15 }}
            transition={{ duration: 0.2 }}
            className="bg-white border border-slate-200 rounded-[1.8rem] shadow-sm overflow-hidden p-2 sm:p-5"
          >
            <PastPapersView />
          </motion.div>
        )}
