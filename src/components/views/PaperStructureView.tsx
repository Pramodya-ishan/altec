...
export default function PaperStructureView() {
 ...
 setQuizError(errData.error || "The question service is unavailable.");
 }
 } catch {
 setQuizError("The network request failed.");
 } finally {
 setLoadingQuiz(false);
 }
 };
 ...
 saveData(nextData);
 if (showNotification) {
 showNotification(`Quick test complete. Score: ${finalNormalizedScore}/${mcqMax}`, 'success');
 }
 ...
 {isTopicSelectionMode ? "Unit test" : "Question practice"}
 </span>
 <h3 className="text-lg md:text-xl font-display font-bold truncate max-w-lg" title={selectedTopic || "Choose a lesson"}>
 ...
 <h4 className="text-sm font-bold text-slate-800">Choose a lesson for the test</h4>
 ...
 {!isAnswerSubmitted && (
 <div className="flex items-center gap-3 bg-indigo-50/50 border border-indigo-100 rounded-2xl p-3 justify-between ">
 ...
 >
 {currentQuizQuestionIdx < 4 ? "Next question" : "Finish test"}
 </button>
 )}
 </div>
 )}
...
<div className="flex-1 bg-white rounded-2xl p-5 border border-slate-100">
  ...
  <ul className="flex flex-col gap-3">
    {item.topics?.map(topic => (
      <li key={topic} className="flex items-start gap-3 bg-white">
