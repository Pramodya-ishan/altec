...
import { LessonAnalyticsView } from "./LessonAnalyticsView";
...
export function AdmissionPredictorView() {
  ...
  }, [overallZScore, data.zScoreHistory, data, saveData, sftZ, etZ, ictZ]);
  ...
  const chartData = useMemo(() => {
    ...
    const parseDateStr = (dateStr?: string) => {
      ...
      let cleanStr = dateStr.replace(/\s*\(Today\)/ig, "").trim();
      if (cleanStr.includes("-")) {
         const [y, m, d] = cleanStr.split("-").map(Number);
         return new Date(y, m - 1, d, 12, 0, 0); // Noon local time avoids timezone drift
      }
      return new Date(cleanStr); // Fallback for other formats
    };

    const sortedPoints = [...historyPoints].sort((a, b) => parseDateStr(a.date).getTime() - parseDateStr(b.date).getTime());
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      if (daysCount++ > 730) break; // 2 years limit
      ...
      const exactPoints = sortedPoints.filter((p) => {
         ...
         const pFormatted = pD.getFullYear() + "-" + String(pD.getMonth() + 1).padStart(2, "0") + "-" + String(pD.getDate()).padStart(2, "0");
         return pFormatted === dateStr;
      });
      const exactPoint = exactPoints.length > 0 ? exactPoints[exactPoints.length - 1] : undefined;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {(["sft", "et", "ict"] as const).map((sub) => (
          <div key={sub} className="flex flex-col gap-2 relative">
            ...
            />
            {results && (
              <div className="absolute top-1 right-2 text-xs font-bold text-slate-400">
