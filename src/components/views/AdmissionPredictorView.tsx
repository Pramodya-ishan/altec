...
export function AdmissionPredictorView() {
  ...
  useEffect(() => {
    if (isAuthLoading) return; // Prevent overwriting data before user data is loaded
  ...
  }, [overallZScore, data.zScoreHistory, data, saveData, sftZ, etZ, ictZ, isAuthLoading]);
  ...
  const chartData = useMemo(() => {
    ...
    const parseDateStr = (dateStr?: string) => {
      ...
      if (cleanStr.includes("-")) {
         ...
         if (parts.length === 3) {
            const [y, m, d] = parts;
            return new Date(y === 2001 ? 2026 : y, m - 1, d, 12, 0, 0); // Handle 2001 year
         }
      }
