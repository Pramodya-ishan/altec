...
function PulseBlock({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div {...props} className={cn("skeleton-shimmer rounded-2xl bg-slate-100", className)} />;
}
...
export function PageSkeleton({ pathname }: PageSkeletonProps) {
  ...
  if (isChat) {
    return (
      <section className="flex h-full min-h-[70vh] flex-col bg-white px-4 py-6" role="status" aria-busy="true" aria-label="Loading conversation">
  ...
  if (isLibrary) {
    return (
      <section className="space-y-5" role="status" aria-busy="true" aria-label="Loading files">
  ...
  if (isAnalytics) {
    return (
      <section className="space-y-6" role="status" aria-busy="true" aria-label="Loading analytics">
  ...
  return (
    <section className="space-y-6" role="status" aria-busy="true" aria-label="Loading page">
