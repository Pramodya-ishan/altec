...
export function PageSkeleton({ pathname }: PageSkeletonProps) {
  ...
  if (isChat) {
    return (
      <section className="flex h-full min-h-[70vh] flex-col bg-white px-4 py-6" role="status" aria-label="Loading chat">
  ...
  if (isLibrary) {
    return (
      <section className="space-y-5" role="status" aria-label="Loading resources">
